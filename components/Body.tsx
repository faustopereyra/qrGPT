...
const promptSuggestions = [
  'Una vista de ciudad con nubes',
  'Un hermoso glaciar',
  'Un bosque con vista a una montaña',
    ...
    async (values: GenerateFormValues) => {
      ...
      try {
        ...
        if (!response.ok || response.status !== 200) {
          ...
          throw new Error(
            `Fallo al generar QR: ${response.status}, ${text}`,
          );
        }

        const data = await response.json();

        va.track('Codigo QR generado', {
          prompt: values.prompt,
        });

        router.push(`/start/${data.id}`);
      } catch (error) {
        va.track('Fallo al generar QR', {
          prompt: values.prompt,
        });
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)}>
              <div className="flex flex-col gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="https://siliconmind.ai/" {...field} />
                      </FormControl>
                      <FormDescription>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea
                          placeholder="Una vista de ciudad con nubes"
                ...
                />
                <div className="my-2">
                  <p className="text-sm font-medium mb-3">Sugerencias de descripción</p>
